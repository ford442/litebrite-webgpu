            ...
            if neighborPegX >= 0 && neighborPegX < i32(boardWidth) &&
               neighborPegY >= 0 && neighborPegY < i32(boardHeight) {
                ...
                if colorIndex > 0u {
                    ...
                    let distToNeighbor = distance(vec2(f32(x), f32(y)),
                    ...
                    let coreColor = mix(pegColor.rgb, vec3<f32>(1.0, 1.0, 1.0), innerGlow * 0.6);

                    let reflectDir = reflect(-lightDir, normal);
                    let specAngle = max(dot(viewDir, reflectDir), 0.0);
                    let spec = pow(specAngle, 64.0);

                    let fresnel = pow(1.0 - max(dot(normal, viewDir), 0.0), 3.0);

                    var solidColor = coreColor * (0.4 + 0.6 * currentGlowIntensity);
